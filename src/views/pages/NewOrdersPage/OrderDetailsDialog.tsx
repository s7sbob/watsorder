...
const OrderDetailsDialog: React.FC<OrderDetailsDialogProps> = ({ open, orderDetails, onClose }) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth='sm'>
      ...
      <DialogContent>
        {orderDetails && (
          <Box>
            ...
            <Typography>
              <strong>{t('Orders.OrderDetailsDialog.createdAt')}:</strong> {orderDetails.createdAt}
            </Typography>
            ...
            {orderDetails.taxValue !== null && (
              <Typography>
                <strong>{t('Orders.OrderDetailsDialog.taxValue')}:</strong> {orderDetails.taxValue}
              </Typography>
            )}
            {orderDetails.prepTime !== null && (
              <Typography>
                <strong>{t('Orders.OrderDetailsDialog.prepTime')}:</strong> {orderDetails.prepTime}{' '}
                {t('Orders.OrderDetailsDialog.minutes')}
              </Typography>
            )}
      ...
      <DialogActions>
        <Button onClick={onClose}>{t('Orders.OrderDetailsDialog.buttons.close')}</Button>
      </DialogActions>
    </Dialog>
  )
}
